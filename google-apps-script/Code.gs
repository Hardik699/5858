...
function updatePCLaptopSheet(spreadsheet, pcLaptopData, systemAssetsData) {
  ...
  // Headers
  const headers = [
    'PC/Laptop ID', 'Mouse ID', 'Keyboard ID', 'Motherboard ID', 'Camera ID',
    'Headphone ID', 'Power Supply ID', 'Storage ID', 'Storage Type', 'Storage Capacity',
    'RAM Slot 1 ID', 'RAM Slot 1 Size', 'RAM Slot 2 ID', 'RAM Slot 2 Size', 'Total RAM', 'Created Date'
  ];
  ...
  if (pcLaptopData.length > 0) {
    const rows = pcLaptopData.map(item => {
      // Get storage details
      const storageDetails = item.storageId ? systemAssetsData.find(s => s.id === item.storageId) : null;
      
      // Get RAM details
      const ram1Details = item.ramId ? systemAssetsData.find(s => s.id === item.ramId) : null;
      const ram2Details = item.ramId2 ? systemAssetsData.find(s => s.id === item.ramId2) : null;
...
function updateSystemAssetsSheet(spreadsheet, systemAssetsData) {
  ...
  // Headers
  const headers = [
    'Asset ID', 'Category', 'Serial Number', 'Vendor Name', 'Company Name',
    'Purchase Date', 'Warranty End Date', 'RAM Size', 'RAM Type', 'Processor Model',
    'Storage Type', 'Storage Capacity', 'Vonage Number', 'Extension Code', 'Password', 'Created Date'
  ];
...
function updateCategorySheets(spreadsheet, systemAssetsData) {
  ...
  categories.forEach(({ name, category }) => {
    const categoryData = systemAssetsData.filter(asset => asset.category === category);
    ...
    if (categoryData.length > 0) {
      ...
      // Base headers
      let headers = ['Asset ID', 'Serial Number', 'Vendor Name', 'Company Name', 'Purchase Date', 'Warranty End Date', 'Created Date'];
      ...
      const rows = categoryData.map(asset => {
        ...
        if (category === 'ram') {
          ...
          baseData.push(asset.vonageNumber || '-', asset.vonageExtCode || '-', asset.vonagePassword || '-');
        }
